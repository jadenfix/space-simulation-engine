...
printf '%s\n' '[1/6] release build, tests, examples, and receipts'
./scripts/validate.sh

if command -v clang >/dev/null 2>&1; then
  printf '%s\n' '[2/6] Clang warning-as-error build and static analysis'
  make clean
  ...
  printf '%s\n' '[2/6] Clang unavailable; skipped'
fi

if command -v gcc >/dev/null 2>&1; then
  printf '%s\n' '[3/6] GCC warning-as-error build and analyzer'
  make clean
  ...
  printf '%s\n' '[3/6] GCC unavailable; skipped'
fi

if command -v cmake >/dev/null 2>&1; then
  printf '%s\n' '[4/6] independent CMake/CTest build'
  ...
  printf '%s\n' '[4/6] CMake unavailable; skipped'
fi

if [ "${SPACEWIND_SKIP_SANITIZERS:-0}" = 1 ]; then
  printf '%s\n' '[5/6] sanitizers skipped by SPACEWIND_SKIP_SANITIZERS=1'
else
  printf '%s\n' '[5/6] address and undefined-behavior sanitizers'
  ./scripts/sanitize.sh
fi

printf '%s\n' '[6/6] restore and rerun the release artifacts'
./scripts/validate.sh
